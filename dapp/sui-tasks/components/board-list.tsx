...
export function BoardList({ selectedBoardId }: BoardListProps) {
  ...
  const getTaskCount = (boardId: string) => {
    return tasks.filter((task: any) => task.boardId === boardId).length;
  };

  const displayBoards = selectedBoardId
    ? boards.filter((board: any) => board.id === selectedBoardId)
    : boards;
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            {selectedBoardId
              ? displayBoards[0]?.name || "Board"
              : "Your Boards"}
          </h1>
          <p className="text-muted-foreground mt-1">
            {selectedBoardId
              ? displayBoards[0]?.description || ""
                ...
                <CardContent className="space-y-2">
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{getTaskCount(board.id as string)} tasks</span>
                    </div>
                    <div className="flex items-center gap-1">
