...
app.get('/events/moveit/board-migrated', async (req, res) => {
      try {
        ...
        console.error('Failed to fetch moveit-BoardMigrated:', error);
...
app.get('/events/moveit/status-added', async (req, res) => {
      try {
        ...
        console.error('Failed to fetch moveit-StatusAdded:', error);
        res.status(500).json({ error: 'Failed to fetch events' });
      }
    });
...
app.get('/events/moveit/status-removed', async (req, res) => {
      try {
        ...
        console.error('Failed to fetch moveit-StatusRemoved:', error);
        res.status(500).json({ error: 'Failed to fetch events' });
      }
    });
